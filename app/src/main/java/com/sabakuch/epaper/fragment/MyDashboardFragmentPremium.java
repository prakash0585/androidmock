...
import com.sabakuch.epaper.adapter.DashboardExpListAdapter;
...
public class MyDashboardFragmentPremium extends Fragment implements ServiceInterface {
    private static String[] levelType = {"CBSE Class X", "CBSE Class XII", "ICSC Class X"};
    private View rootView;
    private DashboardExpListAdapter listAdapter;
    ...
    private String examIddd,levelIddd;
    public MyDashboardFragmentPremium() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        if (bundle != null)
        {
            examIddd = bundle.getString(Constants.EXAM_ID);
            levelIddd = bundle.getString(Constants.LEVEL_ID);

        }
        setId();
        setExpandableListViewData();
        setupToolbar();
        //  APIAccess.fetchData(MyDashboardFragmentPremium.this, getActivity(), getActivity());
        return rootView;
    }
    ...
    private void setId() {
        ...
        listAdapter = new DashboardExpListAdapter(context, listDataHeader, listData);
...
//                            if (totalPage > page) {
//                                UrlIndex = 0;
//                                APIAccess.fetchPagingData(MyDashboardFragmentPremium.this, getActivity(), getActivity(), true);
//                            } else {
//                            }
//                        }
//                    }
...
//            }
    ...
    private void prepareListData(ArrayList<DashboardData> arrListData) {
        ...
        for (int i = 0; i < arrListData.size(); i++) {
            listDataHeader.add(new DashboardHeaderData(arrListData.get(i).exam_name, arrListData.get(i).exam_date,
                    arrListData.get(i).level_id, arrListData.get(i).last_paper_id,arrListData.get(i).score_id));
         ...
         /*   String level = "";
            ...
            listData.put(listDataHeader.get(i).getLastPaperId(), ("Paper :- " + listDataHeader.get(i).getExamName()) + "\n" + ("Exam Date :- " +
                    examDate) + "\n" + ("Score :- " + listDataHeader.get(i).getScore()));
        }
    ...
    public String httpPost() {
        ...
        try {
            if (UrlIndex == 0) {
                page++;
                response = OpenConnection.callUrl(ServiceUrl.SABAKUCH_PAPER_MY_DASHBOARD + "?feed=1" +  "&user_id=" + strUserID + "&premium_exam=1" +"&examid="+examIddd + "&level_id="+levelIddd);
            } else if (UrlIndex == 1) {
                page = 1;
                response = OpenConnection.callUrl(ServiceUrl.SABAKUCH_PAPER_MY_DASHBOARD + "?feed=1" +  "&user_id=" + strUserID + "&premium_exam=1" +"&examid="+examIddd + "&level_id="+levelIddd);
            }
