...
public class KVPY extends AppCompatActivity implements ServiceInterface {
    private RecyclerView recyclerView;
    private SelectLevelsAdapterKVPYNew adapter;
        ...
        try {
            if (context != null && !context.isFinishing())
                if (CommonUtils.isOnline(context)) {
                    ...
                    UrlIndex = 0;
                    APIAccess.fetchPagingData(KVPY.this, getApplicationContext(), KVPY.this, true);
                } else {
    ...
    public String httpPost() {
        ...
        try {
            if (UrlIndex == 0) {
                response = OpenConnection.callUrl(ServiceUrl.SABAKUCH_USER_DETAIL + "?exam_id=16" + "&user_id=" + strUserID);
            }
            if (UrlIndex == 1) {
                page++;
            ...
            if (SabaKuchParse.getResponseCode(str) != null && SabaKuchParse.getResponseCode(str).equalsIgnoreCase("200")) {
                if (UrlIndex == 0) {
                    ...
                    if (arrData.size() > 0) {
                        ...
                        adapter = new SelectLevelsAdapterKVPYNew(context, arrListData, examId, level1Status, level2Status, level3Status);
