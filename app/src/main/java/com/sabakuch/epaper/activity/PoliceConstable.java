...
public class PoliceConstable extends AppCompatActivity implements ServiceInterface {
    private RecyclerView recyclerView;
    private SelectLevelsAdapterPoliceNew adapter;
        ...
        try {
            if (context != null && !context.isFinishing())
                if (CommonUtils.isOnline(context)) {
                    ...
                    UrlIndex = 0;
                    APIAccess.fetchPagingData(PoliceConstable.this, getApplicationContext(), PoliceConstable.this, true);
                } else {
    ...
    public String httpPost() {
        ...
        try {
            if (UrlIndex == 0) {
                response = OpenConnection.callUrl(ServiceUrl.SABAKUCH_USER_DETAIL + "?exam_id=26" + "&user_id=" + strUserID);
            }
            if (UrlIndex == 1) {
                page++;
            ...
            if (SabaKuchParse.getResponseCode(str) != null && SabaKuchParse.getResponseCode(str).equalsIgnoreCase("200")) {
                if (UrlIndex == 0) {
                    ...
                    if (arrData.size() > 0)
                    {
                        ...
                        adapter = new SelectLevelsAdapterPoliceNew(context, arrListData, examId, level1Status, level2Status, level3Status);
