...
public class InstructionInstActivity extends AppCompatActivity implements ServiceInterface,View.OnClickListener {
    ...
    private static String TAG = InstructionInstActivity.class.getSimpleName();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_instruction_activy);

        context=InstructionInstActivity.this;

        footerSelColor = ContextCompat.getColor(InstructionInstActivity.this, R.color.blue_theme);
        footerUnSelColor = ContextCompat.getColor(InstructionInstActivity.this, R.color.white);
        ...
        try
        {
            if(context!=null&&!context.isFinishing())
                if (CommonUtils.isOnline(context)) {
                    ...
                    APIAccess.fetchPagingData(InstructionInstActivity.this, getApplicationContext(), InstructionInstActivity.this,true);
                }	else {
        ...
        tvStartTest.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (isLogin) {
                    ...
                    OnlineTestPaperSectionFragmentNew fragment = new OnlineTestPaperSectionFragmentNew();
                    Bundle bundle = new Bundle();
                    bundle.putString(Constants.EXAM_ID, "2");
                    bundle.putString(Constants.LEVEL_ID, "1");
                    bundle.putString(Constants.EXAM_TYPE, "2");
                    bundle.putString("INSTITUTE_NAME", "vinayyour");
    ...
    public String httpPost() {
        ...
        try {
            if (UrlIndex == 0) {
                response = OpenConnection.callUrl("https://sabakuch.com/mock_paper/api/instructions?exam_id=2&institute_name=vinayyour&level_id=1");
            }else if(UrlIndex == 1){
                postUserDetail();
        ...
        switch (v.getId()) {
            case R.id.ll_home:
                if (isNavigatingStatus()) {
                    ...
                    setHomeColor();
                  //  startActivity(new Intent(InstructionInstActivity.this,InstructionInstActivity.class));
                    setFragment(new SelectExamsFragment());
                }
                break;
            case R.id.ll_statistics:
                boolean isLogin = CommonUtils.getLoginPreferences(InstructionInstActivity.this, Constants.IS_LOGIN);
                if (isLogin) {
                    if (isNavigatingStatus()) {
                        quitExamAlertDialog(1);
                    } else {
                        setStatisticsColor();
                        setFragment(new MyDashboardFragmentNew());
                    }
                } else {
                    CommonUtils.loginAlertDialog(InstructionInstActivity.this, getResources().getString(R.string.please_login_to_access_this_feature));
                }
                break;
            ...
            case R.id.ll_settings:
                if (isNavigatingStatus()) {
                    ...
                    setSettingsColor();
                    setFragment(new SettingsFragmentNews());
                }
                break;
        }
        getSupportActionBar().setTitle("");
    ...
    private boolean isNavigatingStatus() {
        ...
        return (currentFragment instanceof OnlineTestPaperSectionFragmentNew);
    }


    public void quitExamAlertDialog(final int tabPosition) {
        AlertDialog.Builder builder = new AlertDialog.Builder(InstructionInstActivity.this);
    ...
    private boolean isNavigatingMain() {
        Fragment currentFragment = getSupportFragmentManager().findFragmentById(R.id.container_body);
        return (currentFragment instanceof SelectExamsFragment || currentFragment instanceof MyDashboardFragmentNew
    ...
    private boolean isNavigatingResultScreenStatus() {
        Fragment currentFragment = getSupportFragmentManager().findFragmentById(R.id.container_body);
        return (currentFragment instanceof ResultFragmentNew);
    }
...
//            } else {
            ...
            CommonUtils.saveIntPreferences(InstructionInstActivity.this, Constants.Permission, SabakuchEmockApplication.permission);
...
//                CommonUtils.saveIntPreferences(MainActivity.this, Constants.ReadPermission, 2);
...

    {
        ...
        try {
            ...
            URL url = new URL("https://sabakuch.com/mock_paper/api/instructions?exam_id=2&institute_name=vinayyour&level_id=1");
    ...
    void createMarkersFromJson(String json) throws JSONException {
        ...
        jsonarray = json1.getJSONArray("instructions");

        for (int i = 0; i < jsonarray.length(); i++) {
            JSONObject jsonObject2 = jsonarray.getJSONObject(i);
            String L1 = jsonObject2.getString("level_id");
            String L2 = jsonObject2.getString("exam_id");
              Log.d("####################exm",L2);
              Log.d("####################lvl",L1);
           //   Toast.makeText(this, L1, Toast.LENGTH_SHORT).show();

           /* JSONArray jsonArray1 = new JSONArray(jsonObject2.getString("hearing_details"));
            ArrayList<HearingDetails> hearingDetailses = new ArrayList<>();
            for (int j = 0; j < jsonArray1.length(); j++) {
                JSONObject jsonObject3 = jsonArray1.getJSONObject(j);
                HearingDetails hearingDetails = new HearingDetails();
                hearingDetails.setFirstParty(jsonObject3.getString("first_party"));
                hearingDetails.setSecondParty(jsonObject3.getString("second_party"));
                hearingDetailses.add(hearingDetails);
            }
            stringArrayListMultimap.put(calendarDate, hearingDetailses);*/
        }
